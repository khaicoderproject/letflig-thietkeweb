...
const Castfun = (castee) => {
  ...
  return `<div class="Now_playing_movies castdiv" >
     <img class="poster" data-id="${castee.id}" src="https://image.tmdb.org/t/p/w500/${castee.profile_path}"
        onerror="this.onerror=null;this.src='./resources/D moviesand tv show.png';"
         loading="lazy" alt="${castee.original_name}">
...
const Trailerfunc = function (id) {
  return `<iframe class="youtubePlayer" src="https://player.autoembed.cc/embed/movie/${id}" width="100%" height="100%" loading="lazy" frameborder="0" allowfullscreen></iframe>`;
};

let url = document.location.href;
let fetcid = url.slice(url.indexOf("=") + 1); //lay ra id
console.log(fetcid)
